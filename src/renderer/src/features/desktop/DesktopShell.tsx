...
import { cx, drag, settingsBorder, settingsCard, settingsMuted, settingsPanel, settingsSurface, settingsText } from "../../lib/styles";
...
export function DesktopShell({ settings }: { settings: CopClipSettings }) {
  ...
  return (
    <main className={cx("grid h-screen grid-cols-[190px_minmax(0,1fr)] overflow-hidden rounded-[20px] border border-[var(--settings-shell-border)] shadow-[0_18px_48px_rgb(0_0_0_/_22%)] max-[560px]:grid-cols-1 max-[560px]:rounded-none", settingsSurface)} aria-label="CopClip desktop shell">
      <div className={cx("fixed left-0 right-0 top-0 z-10 h-11", drag)} data-window-drag-region="desktop" aria-hidden="true" />
      <aside className="flex min-w-0 flex-col justify-between border-r border-[var(--settings-sidebar-border)] bg-[image:var(--settings-sidebar)] pb-3.5 pl-3.5 pr-2.5 pt-12 max-[560px]:border-b max-[560px]:border-r-0 max-[560px]:p-[18px]" aria-label="CopClip navigation">
            ...
            {settingsPages.map((page) => {
              ...
              return (
                <button
                  ...
                  className={cx("flex min-h-[42px] w-full items-center gap-2.5 rounded-[10px] border border-transparent px-3 text-left text-[15px] font-semibold transition-colors max-[560px]:min-h-12 max-[560px]:text-lg", selected ? "bg-gradient-to-b from-[var(--settings-nav-active-from)] to-[var(--settings-nav-active-to)] text-white" : "text-[var(--settings-fg)] hover:bg-[var(--settings-nav-hover)]")}
                  key={page.id}
                  type="button"
      ...
      <section className={cx("h-screen min-w-0 overflow-y-auto px-6 pb-9 pt-6 [scrollbar-color:var(--settings-scrollbar)_transparent] [scrollbar-width:thin] max-[560px]:p-[18px]", settingsPanel)}>
        <header className="mb-[18px] flex items-start justify-between gap-3.5 max-[560px]:grid">
          <div>
            <h1 className={cx("text-2xl font-bold tracking-[-0.03em]", settingsText)}>{pageTitle}</h1>
